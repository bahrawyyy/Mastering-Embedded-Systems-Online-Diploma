...
typedef struct
{
	uint16_t 		EXTI_lineNumber;  // EXTI0 --> EXTI15
	GPIO_TypeDef 	*GPIO_Port;
	uint16_t 		GPIO_Pin;     // Same as the first one to limit his options
...
typedef struct
{
	...
	uint8_t 				TriggerCase;	   // Specifies Rising or falling or both trigger
											   // This parameter must be set based on @ref EXTI_Trigger_define

	uint8_t 				IRQ_Enable;		   // Enable or disable the EXTI IRQ (That will enable the IRQ MASK in EXTI and also on the NVIC
											   // This parameter must be set based on @ref EXTI_IRQ_define


	void (*P_IRQ_CallBack)(void);			   // Set the C function that will be called once the IRQ happen
